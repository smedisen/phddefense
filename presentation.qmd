...
footer: "his.se/smeh &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; PhD Defense Presentation"
...
function drawLine(start_point, end_point, ctrl_point1, ctrl_point2, draw_dots = false, draw_arrow = true) {
    ...
    if (draw_arrow) {
        var coord = getBezierXY(0.995, start_point[0], start_point[1], ctrl_point1[0], ctrl_point1[1], ctrl_point2[0], ctrl_point2[1], end_point[0], end_point[1]);
        var angle = getBezierAngle(0.995, start_point[0], start_point[1], ctrl_point1[0], ctrl_point1[1], ctrl_point2[0], ctrl_point2[1], end_point[0], end_point[1]);
...
<canvas id="canvas2" width="650" height="347" style="top: 200px; left: 544px; position: absolute" class="fragment fade-in visible current-fragment" data-fragment-index="0">
</canvas>
...
<script type="text/javascript">
    ...
    ctx.beginPath();
    ctx.moveTo(0, 180*2 +160);
    ctx.lineTo(90*2, 180*2 + 160);
    ctx.lineTo(90*2, 270*2 + 160);
    ...
    start_point = [850, 490-60 + 160];
    end_point = [1193-50, 5+160];
    ctrl_point1 = [(start_point[0]+end_point[0])/2, start_point[1]];
    ctrl_point2 = [end_point[0], (start_point[1]+end_point[1])/2];
    drawLine(start_point, end_point, ctrl_point1, ctrl_point2, false, false);
...
:::
::: {.column width="40%"}
::: {style="font-size: 50%; padding-top: 50px;"}
...
plt.gca().legend(loc="upper right", fancybox=False, frameon=False)
...
# plt.scatter([0.30], [0.55], facecolors='none', edgecolors='#47ff7e', s=2550)
...
<ul>
  <li style="list-style-type: none">
    <Ul>
      ...
      <li>Can *not* easily be transferred</li>
...
<ul>
  <li style="list-style-type: none">
    <Ul>
      ...
      <li>Used both *algorithmically* and for <br>*decision support*</li>
    </ul>
  </li>
</ul>
...
<italic>How can different *representations of knowledge* be extracted from the solutions of multi-objective objective optimization problems, while considering diverse multi-objective optimization problems?</italic>
:::

:::: {.columns}
::: {.column}
* Variable trends 
    * Finding interesting trends for variables in the objective space
    * *Trend Mining 2.0* <psmall>(Paper I)</psmall>

* Variable importance
    * Finding important variables for convergence and diversity
...
* Decision rules
    * Flexible Pattern Mining (*FPM*)  
    [@partb]
    * Finds rules describing the *decision space*
    * *Selected* and *Unselected* sets of solutions in the *objective space*
    * Interactive graph-based visualization <psmall>(Paper III)</psmall>
:::
::::
:::
...
* Trend Mining
  ...
  * How do different *variables change* along different directions in the *objective space*?  
  ...
  2. <lsmall>Projection of solutions on to the reference direction</lsmall>
...
::: {style="font-size: 50%; position: absolute; bottom: 0px;"}
...
* User study
    ...
    * 20 participants, Positive feedback
...
::: {.column}
* Offline KDO
    * Discovery knowledge from *previous* optimizations
    * Apply this knowledge in *future* optimizations
:::

::: {.column}
*  Online KDO
    * Discover knowledge about the *current* optimization
    * Feed this knowledge *back into* the optimization
:::

::: {style="scale:75%; text-align: center;"}
Either form of knowledge can *inform* the decision maker
:::
...
* *Knowledge-driven* mutation operator
  ...
  * Modular, for Preference-based MOEAs
  * R-metric [@li2017r]
  * EH-metric [@ehmetric]

![](figures\onlinekdopaper.png){style="width: 80%; margin-top: -10px; padding-left: 200px;"}

:::
::: {.column width="50%"}

![](figures\kdoflow.drawio.svg){style="width: 90%; margin-left: 2vw;"}
...
* Mimer
  ...
  * Add compatibility with existing optimization engines
:::
::::
...
::: {#refs style="font-size: 55%; padding-top: 50px;"}
:::
